...
class LogAnalyzer:
    ...
    def _prepare_analysis_prompt(
        ...
        original_instructions_section = ""
        if original_prompt:
            # Include the original generation prompt for full context
            original_instructions_section = f"""ORIGINAL CODE GENERATION INSTRUCTIONS:
...
"""
        ...
        if code_context:
            ...
            code_section = f"""CODE THAT WAS EXECUTED:
```python
...
"""
        ...
        # Load and format the task prompt from external file
        prompt = PromptManager().load_task_prompt(
            "log_analysis_task",
            test_objective=test_objective,
            workflow_type=workflow_type,
            original_instructions_section=original_instructions_section,
